...
class GetCandleResponsePayloadToListCandleMapperTest {
  ...
  @BeforeEach
  void setUp() {
    final var symbol = CandleFixture.SYMBOL;
    final var precision = CandleFixture.PRECISION;

    dateTimeUtil = mock(DateTimeUtil.class);

    getCandleResponsePayloadToListCandleMapper = GetCandleResponsePayloadToListCandleMapper.builder()
      .symbol(symbol)
      .precision(precision)
